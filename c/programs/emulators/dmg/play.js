...
var keyZones = [
  ...
  ["up", [38]],
  ["down", [40]],
  ["a", [88, 74,      13]],    // numpad enter
  ["b", [90, 81, 89,  99]],    // numpad 3
  ["select", [16,     96]],    // numpad 0
  ["start", [32,      110]]    // numpad .
];
var cout = function() {
  ...
  binaryHandle.onload = function () {
    if (this.readyState === 2) {
      try {
        ...
        settings[13] = false;
        console.log(1, GameBoyEmulatorInitialized(), gameboy, settings);
...
function loadViaXHR () {
	$state.loading();
  if ($url.query.rom) {
    loadRom( $url.query.rom.indexOf('/') == 0 ? $url.query.rom : '/' + $url.query.rom );
  } else {
...
function windowingInitialize() {
	...
	mainCanvas = document.getElementById("origin");
